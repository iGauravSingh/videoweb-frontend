...
const PlanCrd = ({ plan, setSelectedSession, selectedSession }: PlanProps) => {
  const { name, price, canDownload, canWatchSouth } = plan;
  ...
  return (
    ...
    >
      <div className="rounded bg-gradient-to-r from-cyan-500 to-blue-500 w-full p-3 text-white font-bold">
        ...
        <p className="font-light">₹{price.amount / 100}</p>
      </div>
      ...
      <div className="border-b py-4 flex text-reg items-center">
        {canDownload ? (
          <div className="w-6 h-6 rounded-full text-sm bg-cyan-500 flex items-center justify-center">
            <CheckIcon color="while" width={15} fontWeight={900} />
          </div>
        ) : (
          <div className="w-6 h-6 rounded-full text-sm bg-red-500 flex items-center justify-center">
            <XMarkIcon color="while" width={15} fontWeight={900} />
          </div>
        )}
        <div className="ml-3">
          <h3 className="text-gray-600">Monthly Price</h3>
          <h3 className="font-semibold">₹{price.amount / 100}</h3>
        </div>
      </div>
      <div className="border-b py-4 flex text-reg items-center">
        {canDownload ? (
      ...
      <div className="border-b py-4 flex text-reg items-center">
        ...
        <div className="ml-3">
          ...
          <h3 className="font-semibold">
            {canWatchSouth ? "Included" : "Not Included"}
          </h3>
        </div>
      </div>
    </div>
